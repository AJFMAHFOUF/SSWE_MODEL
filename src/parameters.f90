module params
 ...
 real, parameter    :: kdiff = 0.00E16        ! Coefficient for horizontal diffusion
 real, parameter    :: dt = 1800.0            ! model time step
 integer, parameter :: nhtot = 24*30          ! number of hours of model integration
 integer, parameter :: npdt = nhtot*3600/dt   ! number of model time steps
 integer, parameter :: nfreq = 5*24*3600/dt   ! hourly output archiving frequency
 character(len=3)   :: expid='tr2'            ! experiment identifier
 logical            :: lreaduv=.false.        ! logical to use u v at initial time
